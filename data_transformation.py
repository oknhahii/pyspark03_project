...
def data_report1(df_city_sel, df_presc_sel):
    try:
        loggers.warning("processing the data_report1 about them presc by the city  ..")
        ...
        df_presc_grp = df_presc_sel.groupBy(df_presc_sel.presc_state, df_presc_sel.presc_city).agg(
            countDistinct("presc_id").alias('presc_counts'), sum("tx_cnt").alias('tx_sum'))

        loggers.warning("Don't report a city if no prescriber is assigned to it.....lets join df_city_sel and "
                        "df_presc_grp")

        df_city_presc_join = df_city_split.join(df_presc_grp, (df_city_sel.state_id == df_presc_grp.presc_state) & (
                df_city_sel.city == df_presc_grp.presc_city), 'inner')

        df_final = df_city_presc_join.select("city", "state_name", "county_name", "population", "zipcounts", "presc_counts")
...
def data_report2(df_presc_sel):
    try:
        loggers.warning('executing data_report about the prescriber who has more than 20 yoe and less than 50 yoe and rank top 5 base on their tx_cnt for each state')
        ...
        loggers.warning("data_report2 method executed...., go")
